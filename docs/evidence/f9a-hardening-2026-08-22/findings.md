...
Status: **LOCAL GATE LULUS — REGISTER MENUNGGU REMOTE GATE**
...
| F9A-LOCAL-003 | P2 | Koreksi clock WSL/Docker Desktop menghasilkan minimum negatif sporadis pada custom timing k6 dan output durasi migration, tanpa error HTTP atau kegagalan rekonsiliasi. | Gate memakai p95, error counter, server result, dan rekonsiliasi; hasil minimum tidak dipakai sebagai acceptance metric. Workflow manual Linux native wajib mengonfirmasi baseline sebelum penutupan. | Engineering | Local p95/reconciliation 2026-08-22 lulus; retest remote wajib |

P0 terbuka: 0. P1 terbuka: 0 berdasarkan seluruh gate lokal. Jumlah final hanya boleh dikunci setelah remote CI dan workflow manual baseline selesai.
